...
void catan::Player::implementDevelopCard(DevelopmentCard *d)
{
  ...
  if (d->getType() == "monopol card")
  {
    ...
    { // go over all the card in oder to find the less amount.
    ...
    for (Player *p : this->_otherPlayers)
    {                                  // go over the other players
  ...
  if (d->getType() == "year of plenty card") // find the 2 card withthe min amount- and add them to the player's dev cards
  {
...
{
  if (this->_board->getJunctionAt(JunctionNumber)->getis_occupate() == false) // cheach if the junction is clear
  {
    cout << this->getName() << " Placed First Settelment at junction number: " << this->_board->getJunctionAt(JunctionNumber)->getNum() << endl;
    this->_board->getJunctionAt(JunctionNumber)->setis_occupate(true); // set to occupate
  ...
  {
    throw std::logic_error("Error: this Junction is already occupied");
  }
}
...
int Player::rollDice()
{
  if (this->getIsMyTurn())
  {
    ...
    this->_isMyTurn = false;              // finish my turn
    this->_nextPlayer->setIsMyTurn(true); // next player turn
    return ans;
  }
  else
  {
...
void catan::Player::placeSettelment(int juncNumber)
{
  ...
  { // check if the road belong to this player:
    ...
    { // if at the end of the road there is no junction with settelment\town- the player can place settelment.
