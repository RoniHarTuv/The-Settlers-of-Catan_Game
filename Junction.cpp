...
Junction::Junction(int num, vector<Way *> ways, vector<Land *> lands)
    : _num(num), _town(nullptr), _settelment(nullptr),_is_occupate(false), _ways(ways),
      _lands(lands) {}
...
int Junction::getNum() { return _num; }

// Add a Way to the Junction
void Junction::addWay(Way *w) { _ways.push_back(w); }

// Add a Land to the Junction
void Junction::addLand(Land *l) { this->_lands.push_back(l); }

// Get the first Land associated with the Junction
Land *Junction::getLand1() { return !_lands.empty() ? _lands[0] : nullptr; }

// Get the second Land associated with the Junction
Land *Junction::getLand2() { return _lands.size() > 1 ? _lands[1] : nullptr; }

// Get the third Land associated with the Junction
Land *Junction::getLand3() { return _lands.size() > 2 ? _lands[2] : nullptr; }

// Get the first Way associated with the Junction
Way *Junction::getWay1() { return this->_ways[0]; }

// Get the second Way associated with the Junction
Way *Junction::getWay2() { return _ways[1] == nullptr ? nullptr : _ways[1]; }

// Get the third Way associated with the Junction
Way *Junction::getWay3() { return _ways[2] == nullptr ? nullptr : _ways[2]; }
...
bool Junction::getis_occupate() { return this->_is_occupate; }

// Set the occupied status of the Junction
void Junction::setis_occupate(bool b) { this->_is_occupate = b; }
...
Way::Way(int n, Junction *j1, Junction *j2) : _road(nullptr) {
  this->_num = n;
...
Way::~Way() { delete _road; }

// Get the first Junction associated with the Way
Junction *Way::getJunction1() { return this->_junctions[0]; }

// Get the second Junction associated with the Way
Junction *Way::getJunction2() { return this->_junctions[1]; }
...
Road *Way::getRoad() { return this->_road; }

// Get the number of the Way
int Way::getNum() { return this->_num; }

// Print information about the Way
void Way::printWay() {
  cout << "Way number: " << this->getNum() << " is occupied? "
       << this->getis_occupate() << endl;
}

// Check if the Way is occupied
bool Way::getis_occupate() { return this->is_occupate; }

// Set the occupied status of the Way
void Way::setis_occupate(bool b) { this->is_occupate = b; }
} 
